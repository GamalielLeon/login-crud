...
export class SignUpFormComponent implements OnInit {
  ...
  private userCreated(user: UserModel): void {
    const formDataTemp = this.signUpForm.controls;
    const credentials: LoginModel = {email: formDataTemp.email.value, password: formDataTemp.password.value};
    this.tokenService.getToken(credentials).
      subscribe((tokenData: TokenModel) => this.onTokenCreated(user, tokenData));
  }
